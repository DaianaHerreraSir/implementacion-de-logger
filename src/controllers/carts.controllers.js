...
createCart = async (req, res) => {
  try {
    ...
    if (!newCart) {
      req.logger.error('Error al crear el carrito. newCart es null o undefined.');
    ...
    if (!updatedUser) {
      req.logger.error('Error al actualizar el campo cartID del usuario.');
...
getCart =async (req, res) => {
    ...
    try {
      ...
      req.logger.info("Obtener carrito por ID:", cid, cart); 
...
addProductToCart = async (req, res) => {
  try {
    ...
    const userId = req.user._id;
    req.logger.info("ID de usuario para agregar producto al carrito:", userId);
